// rewrite code for clarity and incorporate best practise and understanding
class Solution
{
public:
...
{
public:
    double dfs(string src, string dest, unordered_map<string, vector<pair<string, double>>> adjList, unordered_map<string, int> vis)
    {
        if (src == dest)
            return 1;
        vis[src] = 1;
        ...
        for (auto p : adjList[src])
        {
            if (!vis[p.first])
            {
                int temp = dfs(p.first, dest, adjList, vis);
                if (temp != -1)
                    return (p.second * dfs(p.first, dest, adjList, vis));
            }
        }
    ...
    vector<double> calcEquation(vector<vector<string>> &equations, vector<double> &values, vector<vector<string>> &queries)
    {
        ...
        for (int i = 0; i < queries.size(); i++)
        {
            ...
            if (adjList.find(queries[i][0]) == adjList.end() || adjList.find(queries[i][1]) == adjList.end())
                ...
                res.push_back(dfs(queries[i][0], queries[i][1], adjList, vis));
        }
